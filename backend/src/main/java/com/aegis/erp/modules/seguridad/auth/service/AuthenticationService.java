...
private final UsuarioRepository usuarios;private final StatusUsuarioRepository statuses;private final PasswordEncoder encoder;private final AccessAuditService audit;private final UsuarioAuthMapper mapper;private final Clock clock;
public AuthenticationService(UsuarioRepository usuarios,StatusUsuarioRepository statuses,PasswordEncoder encoder,AccessAuditService audit,UsuarioAuthMapper mapper,Clock clock){this.usuarios=usuarios;this.statuses=statuses;this.encoder=encoder;this.audit=audit;this.mapper=mapper;this.clock=clock;}
...
usuario.registrarIngreso(LocalDateTime.now(clock));audit.registrar(usuario.getIdUsuario(),CONCEDIDO,context);return mapper.toLoginResponse(usuario);}
