...
public class AccessAuditService {
    ...
    public AccessAuditService(
            TipoAccesoRepository tipos, BitacoraAccesoRepository bitacora, Clock clock) {
        this.tipos = tipos;
    ...
    public void registrar(String idUsuario, String tipoNombre, LoginClientContext c) {
        TipoAcceso tipo =
                tipos.findByNombre(tipoNombre)
                        .orElseThrow(
                                () ->
                                        new IllegalStateException(
                                                "Catálogo TIPO_ACCESO incompleto."));
        bitacora.save(
                new BitacoraAcceso(
                        limit(idUsuario, 50),
                        tipo,
                        LocalDateTime.now(clock),
                        limit(c.userAgent(), 200),
                        limit(c.direccionIp(), 50),
                        tipoNombre,
                        limit(c.sesion(), 100)));
    }
