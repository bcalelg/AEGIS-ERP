...
@BeforeEach void setup(){service=new AuthenticationService(usuarios,statuses,encoder,audit,mapper,clock);}
@Test void loginExitoso(){Usuario u=usuario(activo,2,5);when(usuarios.findForAuthentication("Administrador")).thenReturn(Optional.of(u));when(encoder.matches("ITAdmin","hash")).thenReturn(true);assertThat(service.login(new LoginRequest("Administrador","ITAdmin"),context)).isEqualTo(new LoginResponse(true,"Administrador","Administrador","IT","Administrador",true));assertThat(u.getIntentosAcceso()).isZero();assertThat(u.getUltimaFechaIngreso()).isEqualTo(LocalDateTime.now(clock));verify(audit).registrar("Administrador",AuthenticationService.CONCEDIDO,context);}
