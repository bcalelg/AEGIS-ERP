...
@ParameterizedTest
@ValueSource(strings={"'","''","' OR '1'='1","' OR 1=1 --","admin' --","' OR 'a'='a","\" OR \"1\"=\"1","') OR ('1'='1","1 OR 1=1"})
void payloadsSqlInjectionNoAutenticanNiCreanCookie(String payload)throws Exception{when(authenticationService.login(any(),any())).thenThrow(new com.aegis.erp.common.exception.InvalidCredentialsException());String body="{\"idUsuario\":\""+jsonEscape(payload)+"\",\"password\":\"cualquier valor\"}";mvc.perform(post("/api/auth/login").with(csrf()).contentType(MediaType.APPLICATION_JSON).content(body)).andExpect(status().isUnauthorized()).andExpect(header().doesNotExist("Set-Cookie")).andExpect(jsonPath("$.detail").value(com.aegis.erp.common.exception.InvalidCredentialsException.GENERIC_MESSAGE)).andExpect(jsonPath("$.detail").value(org.hamcrest.Matchers.not(org.hamcrest.Matchers.containsString("ORA-"))));}

@Test void payloadSqlInjectionEnPasswordNoAutenticaNiCreaCookie()throws Exception{when(authenticationService.login(any(),any())).thenThrow(new com.aegis.erp.common.exception.InvalidCredentialsException());mvc.perform(post("/api/auth/login").with(csrf()).contentType(MediaType.APPLICATION_JSON).content("{\"idUsuario\":\"Administrador\",\"password\":\"' OR '1'='1\"}")).andExpect(status().isUnauthorized()).andExpect(header().doesNotExist("Set-Cookie")).andExpect(jsonPath("$.detail").value(com.aegis.erp.common.exception.InvalidCredentialsException.GENERIC_MESSAGE));}
