...
export class StatusUsuarioService {
  ...
  private readonly url = `${environment.apiUrl}/security/status-usuarios`;

  list() {
    return this.http.get<StatusUsuario[]>(this.url);
  }

  get(id: number) {
    return this.http.get<StatusUsuario>(`${this.url}/${id}`);
  }

  create(request: StatusUsuarioRequest) {
    return this.http.post<StatusUsuario>(this.url, request);
  }

  update(id: number, request: StatusUsuarioRequest) {
    return this.http.put<StatusUsuario>(`${this.url}/${id}`, request);
  }
  ...
  print(search: string) {
    return this.http.get<StatusUsuario[]>(`${this.url}/print`, {
