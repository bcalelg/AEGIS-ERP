...
export class RoleService {
  ...
  private readonly url = `${environment.apiUrl}/security/roles`;
  ...
  create(request: RoleRequest) {
    return this.http.post<Role>(this.url, request);
  }

  update(id: number, request: RoleRequest) {
    return this.http.put<Role>(`${this.url}/${id}`, request);
  }
  ...
  print(search: string) {
    return this.http.get<Role[]>(`${this.url}/print`, { params: this.searchParams(search) });
  }
